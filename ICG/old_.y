...
assignment_stmt:			T_IDENTIFIER 
							T_EQUAL 
							expression_stmt 
							{if($<data.val_type>3==string_type || $<data.val_type>3==list_type){add_variable(current_scope, $<data.idval>1, $<data.val_type>3, curr_line, (void*)(($<data.sval>3))); } else{add_variable(current_scope, $<data.idval>1, $<data.val_type>3, curr_line, (void*)(&($<data.numval>3)));} sprintf($<data.code>$, "%s\n%s = %s",$<data.code>3, $<data.idval>1, $<data.temp>3);}
...
condition:					condition
						...
						|	{$<data.next>$ = newLabel("label"); $<data.true>$ = $<data.true>0; $<data.false>$ = 
							$<data.next>$; push(&breakpoint, $<data.next>$);}
							condition_and
...
for_stmt:					T_FOR 
							...
							range_stmt 
							T_COLON 
							{add_variable(current_scope, $<data.idval>2, int_type, curr_line, (void*)(&temp));}
